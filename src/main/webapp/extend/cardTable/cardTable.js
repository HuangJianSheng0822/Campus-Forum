layui.define(['table', 'laypage','jquery', 'element'], function(exports) {
	...
	var defaultOption = {
		...
		url: "",// 数据 url 连接
		loading: true,//是否加载
		limit: 16, //每页数量默认是每行数量的双倍
		linenum: 4, //每行数量 2,3,4,6
		currentPage: 1,//当前页
		data:[],       //静态数据
		limits:[],     //页码
		page: true, //是否分页
		layout: ['prev', 'page', 'next'],//分页控件
		request: {
			pageName: 'page' //页码的参数名称，默认：page
			, limitName: 'limit' //每页数据量的参数名，默认：limit
			, idName: 'id'       //主键名称，默认：id
			, titleName: 'text' //标题名称，默认：title
			, imageName: 'image' //图片地址，默认：image
			, remarkName: 'status' //备注名称，默认：remark
			, timeName: 'date' //时间名称，默认：time
		},
		response: {
	...
	function initData(tempData, option) {
		...
		for (var i = 0; i < dataList.length; i++) {
			...
			if (dataList[i][option.request.remarkName]==-1){
				item.remark='暂未找到'
			}else{
				item.remark='失主已找到'
			}
	...
	// 		shadeClose: true,
	// 		area: ['100%', '100%'],
	// 		content: 'comment.jsp?id='+item.id+'&image='+item.image+'&title='+item.title+'&remark='+item.remark+'&time='+item.time,
	// 	});
	// 	layer.full(index);
	// //	---------------------------------
	}
