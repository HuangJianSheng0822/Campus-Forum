layui.define(['table', 'laypage','jquery', 'element'], function(exports) {
	...
	var defaultOption = {
		...
		request: {
			...
			, idName: 'conId'       //主键名称，默认：id
			, titleName: 'title' //标题名称，默认：title
			, imageName: 'image' //图片地址，默认：image
			, remarkName: 'price' //备注名称，默认：remark
			, timeName: 'beginDate' //时间名称，默认：time
		},
		response: {
	...
	function initData(tempData, option) {
		...
		for (var i = 0; i < dataList.length; i++) {
			...
			item.remark= dataList[i][option.request.remarkName]+'元'
	...
	window.cardTableCheckedCard = function (elem,obj) {
		...
		// 	maxmin:false,
		// 	shadeClose: true,
		// 	area: ['20px', '20px'],  //弹出层页面比例
		// 	content: 'comment.jsp?id='+item.id
		// });
		window.open('newComment.jsp?id='+item.id)
